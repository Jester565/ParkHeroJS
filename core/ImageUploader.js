...
function uploadFromStream(bucket, key, s3Client) {
...
async function uploadImageDataOfSizes(data, imgSizes, bucket, objKeyPrefix, s3Client) {
    var bufferStream = new stream.PassThrough();
    bufferStream.end(new Buffer(data));

    await uploadImageStreamOfSizes(bufferStream, imgSizes, bucket, objKeyPrefix, s3Client);
}

//Upload all variations of ride image to s3
async function uploadImageStreamOfSizes(stream, imgSizes, bucket, objKeyPrefix, s3Client) {
    var promises = [];
    ...
    imgSizes.forEach((imgSize, i) => {
        ...
        var upload = uploadFromStream(bucket, key, s3Client);
